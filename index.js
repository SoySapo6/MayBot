...
// --- CONFIGURACIÓN OPTIMIZADA ANTI-BAN Y VELOCIDAD ---
const connectionOptions = {
    logger: pino({ level: 'silent' }),
    printQRInTerminal: opcion == '1' ? true : methodCodeQR ? true : false,
    mobile: MethodMobile,
    // Cambio de navegador para reducir tasa de bloqueo
    ...
    // Desactivado para mayor velocidad y menos consumo de datos (parece menos bot)
    ...
    defaultQueryTimeoutMs: 40000, // Optimización de ping
    connectTimeoutMs: 60000,
    retryRequestDelayMs: 2000,
    cachedGroupMetadata: (jid) => global.conn.chats[jid] ?? {},
    version: version,
    keepAliveIntervalMs: 30000, // Ping más frecuente para mantener conexión viva
    maxIdleTimeMs: 60000,
    // Parche para evitar baneos por mensajes mal formados
...
global.reloadHandler = async function (restatConn) {
    ...
    // --- MODIFICACIÓN: WRAPPER PARA STATUS ESCRIBIENDO ---
    // Esto intercepta los mensajes, pone el estado "Escribiendo" mientras procesa y luego lo quita.
    const wrappedHandler = async (chatUpdate) => {
        try {
            ...
            // Solo actuar en chats reales (no broadcasts)
            if (jid && !jid.includes('status@broadcast')) {
                // Pone el estado escribiendo
                await conn.sendPresenceUpdate('composing', jid);
                
                // Ejecuta el handler original (procesa el comando)
                await conn.handler(chatUpdate);
                
                // Quita el estado escribiendo
    ...
    // Usamos wrappedHandler en lugar de conn.handler directo
    conn.ev.on('messages.upsert', wrappedHandler);
